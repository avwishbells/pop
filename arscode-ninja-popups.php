...
//                        var_dump($decoded);
        ...
        elseif ($ml_manager == 'newsletter' && class_exists('Newsletter'))
        {
            ...
            $res = $api->subscribe($_POST['email'], $names['first'], $names['last'], $ml_newsletter_listid, $cf_data);
        ...
        elseif ($ml_manager == 'mymail')
        {
        ...
        } elseif ($ml_manager == 'mailster') {
            $userdata = array(
                'firstname' => $names['first'],
                'lastname' => $names['last']
            );

            $ml_mailster_list = $POPUP_META['snp_ml_mailster_list'][0];
            if (!$ml_mailster_list) {
            ...
            if (function_exists('mailster')) {
                ...
                $return = mailster_subscribe($_POST['email'], $userdata, $lists);
                if (!is_wp_error($return)) {
                    $Done = 1;
                } else {
                    $api_error_msg = 'Mailster Problem!';
            ...
            try {
                ...
                //print_r($forms->get_form_fields($formId));
        ...
        } elseif ($ml_manager == 'jetpack') {
            $subscribe = Jetpack_Subscriptions::subscribe($_POST['email'], 0, false);
